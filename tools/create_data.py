...
def waymo_data_prep_raw_frame(root_path,
                    info_prefix,
                    version,
                    out_dir,
                    workers,
                    max_sweeps=5):
    """Prepare the pef frame file for waymo dataset, we keep the frame structure the same as the raw data.
    ...
    for i, split in enumerate(splits):
        ...
        save_dir = osp.join(out_dir, 'waymo_raw', split)
        converter = waymo.Waymo(
            load_dir,
            save_dir,
            prefix=str(i),
            workers=workers,
...
if __name__ == '__main__':
    if args.dataset == 'kitti':
        ...
        sunrgbd_data_prep(
            ...
            out_dir=args.out_dir,
            workers=args.workers)
    elif args.dataset == 'waymo_raw':
        waymo_data_prep_raw_frame(
            root_path=args.root_path,
            info_prefix=args.extra_tag,
            version=args.version,
            out_dir=args.out_dir,
