...
from waymo_open_dataset.utils.frame_utils import \
    parse_range_image_and_camera_projection, convert_frame_to_dict

def global_vel_to_ref(vel, global_from_ref_rotation, use_z=False):
    ...
    if use_z:
        ...
        vel = [vel[0], vel[1], 0]
    ref = np.dot(global_from_ref_rotation.T, vel) 
    if use_z:
...
class Waymo2KITTI(object):
    ...
    def save_label(self, frame, file_idx, frame_idx):
        ...
        for obj in frame.laser_labels:
            ...
            track_id = obj.id
            if self.save_speed:
                # inspired by centerpoint
                # https://github.com/tianweiy/CenterPoint/blob/master/det3d/datasets/waymo/waymo_decoder.py#L52
                speed = [obj.metadata.speed_x, obj.metadata.speed_y, 0.0]
                ref_pose = np.reshape(np.array(frame.pose.transform), [4, 4])
                global_from_ref_rotation = ref_pose[:3, :3] 
                ref_speed = global_vel_to_ref(speed, global_from_ref_rotation)
                # note: speed is in ego cooredinate , while the bbox are in camera coordinate
                speed_line = "{} {} {}".format(round(ref_speed[0], 2), round(ref_speed[1], 2), round(ref_speed[2], 2))
            ...
            if self.save_track_id:
                line = line[:-1] + ' ' + track_id + '\n'
            if self.save_speed:
                line = line[:-1] + ' ' + speed_line + '\n'
            line_all = line
...
class Waymo(Waymo2KITTI):
     """Waymo converter.

    This class serves as the converter to extract per frame data from waymo raw data.
    We follow the coding way {a}{bbb}{ccc} to name the data for each frame as done in Waymo2KITTI converter.
    where a is the prefix for different split (0 for training, 1 for validation and 2 for testing), bbb for segment index and ccc for frame index.

    Args:
        load_dir (str): Directory to load waymo raw data.
        save_dir (str): Directory to save per frame data.
        prefix (str): Prefix of filename. In general, 0 for training, 1 for
            validation and 2 for testing.
        workers (str): Number of workers for the parallel process.
        test_mode (bool): Whether in the test_mode. Default: False.
    """
     ...
     def convert_one(self, file_idx):
        ...
        for frame_idx, data in enumerate(dataset):
            ...
            frame_dict = convert_frame_to_dict(frame)

            for camera in self.LiDAR_NAME_LIST:
                inclination = frame_dict[f"{camera}_BEAM_INCLINATION"]  # H,
                extrinsic = frame_dict[f"{camera}_LIDAR_EXTRINSIC"]  # 4,4
                ri0 = frame_dict[
                    f"{camera}_RANGE_IMAGE_FIRST_RETURN"
                ]  # H,W,6; range, intensity, elongation, x, y, z
                ri1 = frame_dict[f"{camera}_RANGE_IMAGE_SECOND_RETURN"]
                # fix rolling shutter, convert the raw range image to virtual frame, merging two returns
                h, w = ri0.shape[:2]

                ri0_points = ri0[..., 3:].reshape((-1,3))  # H*W,3
                ri1_points = ri1[..., 3:].reshape((-1,3))  # H*W,3
                ri0_mask = ri0[..., 0].reshape((-1,)) > 0
                ri1_mask = ri1[..., 0].reshape((-1,)) > 0

                points0_tf = tf.reshape(
                    tf.convert_to_tensor(value=ri0_points[ri0_mask]), [1, -1, 3]
                )
                points1_tf = tf.reshape(
                    tf.convert_to_tensor(value=ri1_points[ri1_mask]), [1, -1, 3]
                )
                points_tf = tf.concat([points0_tf, points1_tf], axis=1)  # 1, num_points, 3
                extrinsic_tf = tf.reshape(
                    tf.convert_to_tensor(value=extrinsic), [1, 4, 4]
                )
                inclination_tf = tf.reshape(
                    tf.convert_to_tensor(value=inclination), [1, h]
                )
                inclination_tf = tf.reverse(inclination_tf, axis=[1])
                range_image_size = [h, w]
                ri, _, _ = range_image_utils.build_range_image_from_point_cloud(
                    points_tf,
                    tf.constant([points_tf.shape[1]]),
                    extrinsic_tf,
                    inclination_tf,
                    range_image_size,
                )
                ri = ri.numpy()[0]  # H,W
                frame_dict[f"{camera}_RANGE_IMAGE_MERGE_VIRTUAL"] = ri

            frame_path = f'{self.save_dir}/{self.prefix}' + \
            f'{str(file_idx).zfill(3)}{str(frame_idx).zfill(3)}.pkl'
            mmcv.dump(frame_dict, frame_path)
