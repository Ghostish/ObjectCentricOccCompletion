...
def get_label_anno(label_path,with_track_id=False,with_speed=False):
    annotations = {}
    annotations.update({
        'name': [],
        'truncated': [],
    ...
    if with_track_id:
        annotations['track_id'] = np.array([x[16] for x in content])
    if with_speed:
        assert with_track_id
        annotations['speed'] = np.array([[float(info) for info in x[17:20]]
                                        for x in content]).reshape(-1, 3)
...
def get_waymo_image_info(path,
                         ...
                         with_speed=False):
    """
    Modified by Zenn, add with_track_id flag
    ...
    def map_func(idx):
        ...
        if label_info:
            ...
            if relative_path:
                label_path = str(root_path / label_path)
            annotations = get_label_anno(label_path,with_track_id,with_speed)
        info['image'] = image_info
        info['point_cloud'] = pc_info
        if calib:
            calib_path = get_calib_path(
