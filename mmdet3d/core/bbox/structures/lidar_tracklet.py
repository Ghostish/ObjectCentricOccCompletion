...
class LiDARTracklet(object):
    ...
    def update_from_prediction(self, boxes, scores, labels, valid_mask, to_numpy=True,to_ego=True):
    ...
    def shared2ego(self, boxes=None, inplace=False):
        ...
        # except RuntimeError:
        #     # a workaround for the bug when calling torch.linalg.inv in 4090 GPUs
        world2tgt_pose = torch.linalg.inv(tgt_pose.to('cpu')).to(src_pose.device)
