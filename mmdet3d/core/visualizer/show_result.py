...
def show_result(
    ...
    if points is not None:
        _write_obj(points, osp.join(result_path, f"{filename}_points{suffix}.obj"))
    if occ_points is not None:
        _write_obj(occ_points, osp.join(result_path, f"{filename}_occ{suffix}.obj"))
    ...
    if gt_bboxes is not None:
        ...
        _write_oriented_bbox(
            gt_bboxes, osp.join(result_path, f"{filename}_gt{suffix}.obj")
        )
    ...
    if pred_bboxes is not None:
        ...
        pred_bboxes[:, 6] *= -1
        _write_oriented_bbox(
            pred_bboxes, osp.join(result_path, f"{filename}_pred{suffix}.obj")
        )
