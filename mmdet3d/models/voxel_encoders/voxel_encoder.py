...
from mmdet3d.ops import make_sparse_convmodule, scatter_v2
...
@VOXEL_ENCODERS.register_module()
class DynamicRangeScatterVFE(DynamicScatterVFE):
    """ Same with DynamicScatterVFE but the point cloud range is dynamic.
      the voxel center should be computed using dynamic point cloud bounds
    """
    ...
    def forward(self,
        ...
        if self._with_voxel_center:
            f_center = features.new_zeros(size=(features.size(0), 3))
            f_center[:, 0] = features[:, 0] - (
                coors[:, 3].type_as(features) * self.vx + pts_min_bounds[:,0])
            f_center[:, 1] = features[:, 1] - (
                coors[:, 2].type_as(features) * self.vy + pts_min_bounds[:,1])
            f_center[:, 2] = features[:, 2] - (
                coors[:, 1].type_as(features) * self.vz + pts_min_bounds[:,2])
            features_ls.append(f_center)
