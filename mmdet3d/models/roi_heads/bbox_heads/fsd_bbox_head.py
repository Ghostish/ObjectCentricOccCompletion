...
class FullySparseBboxHead(BaseModule):
    def __init__(
        ...
        self.return_roi_feats = return_roi_feats
    ...
    def forward(self, pts_xyz, pts_features, pts_info, roi_inds, rois):
        ...
        # note cd: the returned final_cluster_feats only consider non-empty rois, and treat all empty rois as one and idx -1.
        # So we need to remap the final_cluster_feats to the original rois, and set the empty rois to 0-feature.
        ...
        if self.return_roi_feats:
            final_cluster_feats = self.align_roi_feature_and_rois(final_cluster_feats, out_coors, len(rois))
            return cls_score, bbox_pred, final_cluster_feats, nonempty_roi_mask
        else:
            return cls_score, bbox_pred, nonempty_roi_mask
        
    def get_preds_from_roi_feats(self,roi_feats):
        cls_score = self.conv_cls(roi_feats)
        bbox_pred = self.conv_reg(roi_feats)
    ...
    def get_nonempty_roi_mask(self, out_coors, num_rois):
        ...
        # note cd: if an roi is assigned to any point, then it is non-empty. Otherwise it is empty.
        out_coors = out_coors[out_coors >= 0]
        nonempty_roi_mask = torch.zeros(
            num_rois, dtype=torch.bool, device=out_coors.device
        )
    ...
    def align_roi_feature_and_rois(self, features, out_coors, num_rois):
        ...
        if not coors_mask.any():
            new_feature[: len(features), :] = (
                features * 0
            )  # pseudo gradient, avoid unused_parameters
            return new_feature
    ...
    def filter_pos_assigned_but_empty_rois(
        self, pos_data, pos_batch_idx, filtered_pos_mask, roi_batch_idx
    ):
        ...
        for b_idx in range(real_bsz):
            ...
            filter_data = pos_data[data_batch_mask][
                torch.nonzero(filtered_pos_mask[roi_batch_mask]).reshape(-1)
            ]
    ...
    def get_targets(self, sampling_results, rcnn_train_cfg, concat=True):
        ...
        """
        pos_bboxes_list = [res.pos_bboxes for res in sampling_results] # positive predictions
        pos_gt_bboxes_list = [res.pos_gt_bboxes for res in sampling_results] # positive ground truth
    ...
    def get_class_wise_box_weights(self, weights, gt_labels, cfg):
        ...
        all_gt_labels = torch.cat(
            [gt_labels, gt_labels.new_full((num_samples - num_pos,), -1)], dim=0
        )
    ...
    def get_multi_class_soft_label(self, ious, pos_gt_labels, cfg):
        ...
        if isinstance(pos_thrs, float):
            ...
            assert isinstance(pos_thrs, (list, tuple)) and isinstance(
                neg_thrs, (list, tuple)
            )
        ...
        all_gt_labels = torch.cat(
            [pos_gt_labels, pos_gt_labels.new_full((num_samples - num_pos,), -1)], dim=0
        )
        ...
        for i in range(self.num_classes):
            ...
            this_label[interval_mask] = (this_ious[interval_mask] - neg_thr_i) / (
                pos_thr_i - neg_thr_i
            )
    ...
    def get_bboxes(
        ...
        if roi_feats is not None:
            assert rois.size(0) == roi_feats.size(0)
        assert (
            isinstance(class_labels, list)
            and isinstance(class_pred, list)
            and len(class_labels) == len(class_pred) == 1
        )
        ...
        if roi_feats is not None:
            roi_feats = roi_feats[valid_roi_mask]
        ...
        for batch_id in range(batch_size):
            ...
            if batch_size == 1:
                ...
                if roi_feats is not None:
                    cur_roi_feats = roi_feats[roi_batch_mask]
            ...
            if roi_feats is not None:
                seletect_roi_feats = cur_roi_feats[selected]

                result_list.append(
                    (
                        img_metas[batch_id]["box_type_3d"](
                            selected_bboxes, selected_bboxes.size(1)
                        ),
                        selected_scores,
    ...
    def multi_class_nms(
        ...
        boxes_for_nms = xywhr2xyxyr(
            input_meta["box_type_3d"](box_preds, box_preds.size(1)).bev
        )
    ...
    def single_class_nms(
        ...
        if class_scores_keep.int().sum() > 0:
            ...
            if nms_thresh is not None:
                ...
                cur_selected = torch.arange(
                    len(original_idxs), device=original_idxs.device, dtype=torch.long
                )
    ...
    def get_bboxes_from_tracklet(
        ...
        for batch_id in range(batch_size):
            ...
            result_list.append(
                (
                    img_metas[batch_id]["box_type_3d"](
                        cur_rcnn_boxes3d, cur_rcnn_boxes3d.size(1)
                    ),
                    cur_cls_score,
                    cur_class_labels,
                    valid_mask_this,
