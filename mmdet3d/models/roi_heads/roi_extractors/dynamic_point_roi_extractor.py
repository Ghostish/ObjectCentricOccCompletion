...
class TrackletPointRoIExtractor(BaseModule):
    ...
    def forward_separate(self, pts_xyz, batch_inds, pts_frame_inds, rois, roi_frame_inds, max_inbox_point=None):
        ...
        assert pts_max_frames <= max_frames, f"{pts_max_frames} > {max_frames}"

        pts_inds = batch_inds * max_frames + pts_frame_inds

        roi_inds = rois[:, 0].int() * max_frames + roi_frame_inds
        #TODO: check whether there should exist duplicated rois, if no, uncomment the following line
        # if not (len(roi_inds) == len(torch.unique(roi_inds))):
        #     pdb.set_trace()
        assert len(roi_inds) == len(torch.unique(roi_inds)) #may exist duplicated rois
        ...
        all_out_xyz = all_pts_info[:, :3]
        all_local_xyz = all_pts_info[:, 3:6] # ego-box coordinate
        all_offset = all_pts_info[:, 6:-1] # offset to box boundry
        is_in_margin = all_pts_info[:, -1] # in extended box or just in the original box
    ...
    def forward_combined(self, pts_xyz, batch_inds, pts_frame_inds, rois, roi_frame_inds, max_inbox_point=None):
        ...
        if isinstance(self.max_all_point, (tuple, list)):
            ...
            max_all_point = self.max_all_point
        # note cd:
        # all_inds indicates the point_idx of the points in the original point cloud
        # all_roi_inds indicates the rois_idx of roi which points belong to
        all_inds, all_roi_inds, all_pts_info = dynamic_point_pool_mixed(
            rois[..., 1:],
            roi_inds,
            pts_xyz,
