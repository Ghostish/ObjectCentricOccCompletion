...
class TrackletRoIHead(Base3DRoIHead):
    ...
    def __init__(self,
        ...
        self.use_roi_frame_inds = use_roi_frame_inds
    ...
    def _bbox_forward_train(self, pts_xyz, pts_feats, pts_batch_idx, pts_frame_inds, sampling_results):
        ...
        rois = bbox3d2roi([res.bboxes for res in sampling_results]) #note cd: [num_rois, 1 + 7 ], add one dimension for batch_idx
        roi_frame_inds = torch.cat([res.bboxes_frame_inds for res in sampling_results]) #note cd: [num_rois, 1] indicates each box's frame id
        roi_scores = torch.cat([res.scores for res in sampling_results]) #note cd: [num_rois, 1] indicates each box's pre-trained detection score
    ...
    def _bbox_forward(self, pts_xyz, pts_feats, pts_batch_idx, pts_frame_inds, rois, roi_scores, roi_frame_inds):
        ...
        # note cd:
        # ext_pts_inds indicates the point_idx of the points in the original unsampled point cloud
        # ext_pts_roi_inds indicates the rois_idx of roi which points belong to
        # ext_pts_info stores the feature of each point
        # ext_pts_info = dict(
        #     local_xyz=all_local_xyz, # note cd: local coordinate centered at the roi center
        #     boundary_offset=all_offset,# note cd: offset to the roi box boundary like LiDAR-RCNN
        #     is_in_margin=is_in_margin, # note cd: whether the point is in the extended box or just in the original box
        # )
    ...
    def _assign_and_sample(self, tracklet_list, candidates_list):
        ...
        for tid in range(len(tracklet_list)):
            ...
            if sampling_result.pos_gt_bboxes.size(1) == 4 and self.train_cfg.get('hack_sampler_bug', False):
                # note cd： 当 pos_gt 为空时，sampler默认会初始化一个size为0的2D的bbox (4D)列表，这里需要把它转成3D box（7D）
    ...
    def _select_one2one_candidates(self, tracklet_list, candidates_list):
        ...
        for trk, candidates in zip(tracklet_list, candidates_list):
            #每个Track有多个candidate track GT
            if len(candidates) == 0:
                out_trks.append(trk.new_empty())
                continue
            #计算当前Track和每个candidate track之间的intersection_ious
            ...
            if self.train_cfg.get('merge_candidates', False):
                ...
                #将TIoU最高的candidate track分配给当前track
