...
@HEADS.register_module()
class GroupCorrectionOccHead(GroupCorrectionHead):
    def __init__(self,
                 num_classes=3,
                 roi_extractor=None,
                 bbox_head=None,
                 ...
                 init_cfg=None,
                 tune_roi=False,
                 residual_down_ratio = 2.0,
                 residual_embed_mlp = [512,512],):
          super().__init__(
                num_classes,
                 roi_extractor,
                 bbox_head,
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        sample_results = self._assign_and_sample(proposal_list, gt_bboxes_3d,
                                                 gt_labels_3d, occ_labels, occ_labels_scores)
        ...
        ###todo: add gt roi feats for occ prediction


        return losses
    def _occ_forward_train(self, pts_xyz, pts_feats, roi_feats, batch_idx, sampling_results):
        
        pos_rois = bbox3d2roi([res.pos_bboxes for res in sampling_results])
        # randomly sample some "empty" locations
        # here we simply assume that the sensor is located at (0, 0, 0)
        # should be improved.
        shoot_to_origin = F.normalize(0 - pts_xyz[:, :3], dim=-1)
        empty_locs = torch.clamp((torch.rand_like(shoot_to_origin[:,0])), min=1e-3) * shoot_to_origin + pts_xyz[:, :3]
        new_pts_xyz = torch.cat([pts_xyz[:, :3], empty_locs], dim=0)
        batch_idx = torch.cat([batch_idx, batch_idx], dim=0)
        real_loc_masks = new_pts_xyz.new_zeros(new_pts_xyz.size(0))
        # empty_loc_masks is used to indicate real occupied locations, which used as the labels to later online training
        real_loc_masks[:pts_xyz.size(0)] = 1 

        ext_pts_inds, ext_pts_roi_inds, ext_pts_info = self.roi_extractor(
            new_pts_xyz[:, :3], # intensity might be in pts_xyz
            batch_idx,
            pos_rois[:, :8],
            batch_size=1 if not self.training else None,
        )

        # find mapping from gt_roi inds to roi inds
        num_pos_total = 0
        num_bboxes_total = 0
        pos2fullroi = pos_rois.new_zeros(pos_rois.size(0))
        for res in sampling_results:
            num_gt = res.pos_gt_bboxes.size(0)
            num_bboxes = res.bboxes.size(0)
            gt_idxs = torch.arange(num_gt, device=pos_rois.device)
            pos2fullroi[num_pos_total:num_pos_total + num_gt] = gt_idxs + num_bboxes_total
            num_pos_total += num_gt
            num_bboxes_total += num_bboxes
    ...
    def _bbox_forward(self, pts_xyz, pts_feats, batch_idx, rois):
        ...
        cls_score, bbox_pred, roi_feats, valid_roi_mask = self.bbox_head(
            new_pts_xyz,
            new_pts_feats,
            ext_pts_info,
            ext_pts_roi_inds,
    ...
    def simple_test(
        ...
        **kwargs):
        ...
        """Simple testing forward function of GroupCorrectionOccHead.
        ...
        ###todo: add occ prediction in head's get_bboxes method
        bbox_list = self.bbox_head.get_bboxes(
            rois,
            bbox_results['cls_score'],
            bbox_results['bbox_pred'],
            bbox_results['valid_roi_mask'],
            bbox_results['roi_feats'], # pass roi feats for occ prediction
            labels_3d,
            cls_preds,
            img_metas,
            cfg=self.test_cfg)
        ...
        # occ_centers_list_cpu = [[occ_centers.cpu() for occ_centers in result[3] ]for result in bbox_list]
        for i, result in enumerate(bbox_results):
            result['occ_pred'] = [occ_centers.cpu() for occ_centers in bbox_list[i][3] ]
        return bbox_results

    def _assign_and_sample(self, proposal_list, gt_bboxes_3d, gt_labels_3d, occ_labels, occ_labels_scores,):
        """Assign and sample proposals for training.
        ...
        Args:
            ...
            occ_labels (list[torch.Tensor]): Ground truth occ labels [ N * K * 4 ]

        Returns:
            list[:obj:`SamplingResult`]: Sampled results of each training
                sample.
        """
        assert len(proposal_list) == len(gt_bboxes_3d) == len(occ_labels)
        sampling_results = []
        # bbox assign
        for batch_idx in range(len(proposal_list)):
            cur_boxes, cur_scores, cur_pd_labels = proposal_list[batch_idx]
            ...
            cur_gt_bboxes = gt_bboxes_3d[batch_idx].to(cur_boxes.device)
            cur_gt_labels = gt_labels_3d[batch_idx]
            cur_occ_label = occ_labels[batch_idx]
            cur_occ_label_scores = occ_labels_scores[batch_idx]
            assert len(cur_gt_bboxes)  == len(cur_occ_label) == len(cur_occ_label_scores), f"{len(cur_gt_bboxes)},{len(cur_occ_label)},{len(cur_occ_label_scores)}"
            ...
            sampling_result = self.bbox_sampler.sample(assign_result,
                                                       ...
                                                       cur_gt_bboxes.tensor,
                                                       cur_gt_labels)
            sampling_result.pos_occ_labels = cur_occ_label[sampling_result.pos_assigned_gt_inds]
            sampling_result.pos_occ_label_scores = cur_occ_label_scores[sampling_result.pos_assigned_gt_inds]
            if sampling_result.bboxes.shape[1] == 7:
                print(cur_boxes.tensor.shape)
            sampling_results.append(sampling_result)
        return sampling_results