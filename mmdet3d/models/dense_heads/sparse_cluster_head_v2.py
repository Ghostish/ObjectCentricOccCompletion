...
class SparseClusterHeadV2(SparseClusterHead):
    ...
    def get_bboxes(self,
        ...
        # for box,_,_  in concat_list:
        #     assert box.tensor.size(1) == 9, f"{box.tensor.size(1)},{box.tensor.shape}"
        return concat_list
    ...
    def _get_bboxes_single(
        ...
        pred_dim = 9 if self.loss_vel is not None else 7
        if len(cls_logits) == 0:
            out_bboxes = reg_preds.new_zeros((0, pred_dim))
            out_bboxes = input_meta['box_type_3d'](out_bboxes,box_dim=pred_dim)
