...
class WaymoDataset(KittiDataset):
    ...
    def __len__(self):
        ...
        """
        if self.use_firstk > 0:
            return self.use_firstk
    ...
    def get_data_info(self, index):
        ...
        if not self.test_mode or not self.incremental_test:
            ...
            if real_num_sweeps < self.num_previous_seeds:
                sweeps = [
                    dict(velodyne_path=pts_filename, pose=info["pose"]),
                ] + sweeps
    ...
    def get_previous_seed_info(self, sweeps, cur_pose):
        ...
        for sweep in sweeps:
            ...
            if self.new_transform:
                seed["gt_bboxes_3d"] = box_frame_transform_v2(
                    seed["gt_bboxes_3d"], sweep["pose"], cur_pose
                )
            else:
                seed["gt_bboxes_3d"] = box_frame_transform(
                    seed["gt_bboxes_3d"], sweep["pose"], cur_pose
                )
...
def box_frame_transform(pre_boxes, pre_pose, cur_pose):
    ...
    corners_in_world = (
        np.einsum("ij,nj->ni", pre2world_rot, pre_corners) + pre2world_trans[None, :]
    )
    corners_in_curr = (
        np.einsum("ij,nj->ni", world2curr_rot, corners_in_world)
        + world2curr_trans[None, :]
    )

    centers_in_world = (
        np.einsum("ij,nj->ni", pre2world_rot, pre_centers) + pre2world_trans[None, :]
    )
    centers_in_curr = (
        np.einsum("ij,nj->ni", world2curr_rot, centers_in_world)
        + world2curr_trans[None, :]
    )
    ...
    transformed_boxes = np.concatenate(
        [centers_in_curr, pre_boxes[:, 3:6], yaw_in_curr[:, None]], axis=1
    )
...
def box_frame_transform_v2(pre_boxes, pre_pose, cur_pose):
    ...
    yaw_in_curr = torch.atan2(
        heading_vector_in_curr[:, 0], heading_vector_in_curr[:, 1]
    )
    ...
    transformed_boxes = LiDARInstance3DBoxes(
        transformed_boxes, box_dim=transformed_boxes.size(1)
    )
    return transformed_boxes


@DATASETS.register_module()
class MultiOccWaymoDataset(MultiSweepsWaymoDataset):
    def __init__(
        self,
        data_root,
        ann_file,
        ...
        if not test_mode and load_occ:
            self.occ_anno_root = occ_anno_root
            self.oid2occ = mmcv.load(oid2occ_file)
    ...
    def get_data_info(self, index):
        ...
        if not self.test_mode:
            ...
            input_dict["ann_info"] = annos
            if self.load_occ:
                track_ids = annos["track_id"]
                occ_infos = [
                    self.parse_occ_anno(self.oid2occ.get(track_id, None))
                    for track_id in track_ids
                ]
                input_dict["occ_infos"] = occ_infos
                assert len(occ_infos) == len(
                    annos["gt_bboxes_3d"]
                ), f"{len(occ_infos)} vs {len(annos['gt_bboxes_3d'])}"
        return input_dict

    def parse_occ_anno(self, occ_ann_info):
        ...
        ##TODO: debug only, overfit:

        # if "10203656353524179475_7625_000_7645_000/1_4.npz" not in occ_label_name:
        #     return dict(
        #         occ_label_name=None,
        #         label_iou=0,
        #         label_trk_length=0,
    ...
    def get_ann_info(self, index):
        ...
        if self.with_velocity:
            gt_velocity = annos["speed"][:, :2]  # only use x,y velocity
            gt_bboxes_3d = np.concatenate([gt_bboxes_3d, gt_velocity], axis=1)

        # convert gt_bboxes_3d to velodyne coordinates, conversion only modifies the xyz and size
        gt_bboxes_3d = CameraInstance3DBoxes(
            gt_bboxes_3d, box_dim=gt_bboxes_3d.shape[-1]
        ).convert_to(self.box_mode_3d, np.linalg.inv(rect @ Trv2c))
        ...
        # add track id
        anns_results["track_id"] = annos["track_id"]
        # print(anns_results['track_id'])
        assert len(anns_results["track_id"]) == len(
            anns_results["gt_bboxes_3d"]
        ), f"{len(anns_results['track_id'])} vs {len(anns_results['bbox'])}"
        return anns_results
    ...
    def _set_sequence_group_flag(self):
        ...
        for idx in range(len(self.data_infos)):
            if idx != 0 and len(self.data_infos[idx]["sweeps"]) == 0:
                ...
                # note cd: potential bugs here when load_interval > 1
...
@DATASETS.register_module()
class WaymoDatasetWithPredOCC(WaymoDataset):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            pcd_limit_range=pcd_limit_range,
            save_training=save_training,
            use_firstk=use_firstk,
        )
        self.idx2contextname = mmcv.load(idx2contextname_file)
        self.idx2timestamp = mmcv.load(idx2timestamp_file)
        self.occ_pred_path = occ_pred_path
        self.per_object_occ = per_object_occ


    def get_data_info(self, index):
        """
        adding occ_pred_filename to the input_dict
        """

        input_dict = super().get_data_info(index)
        idx = os.path.basename(input_dict['pts_filename']).split(".")[0]
        assert idx in self.idx2contextname, f"{idx} not in idx2contextname dict"
        assert idx in self.idx2timestamp, f"{idx} not in idx2timestamp dict"
        segment_name = self.idx2contextname[idx]
        timestamp = self.idx2timestamp[idx]
        if not self.per_object_occ:
            occ_pred_file = os.path.join(self.occ_pred_path, segment_name, f"{timestamp}.bin")
        else:
            occ_pred_file = os.path.join(self.occ_pred_path, segment_name, f"{timestamp}")
        input_dict["occ_pred_filename"] = occ_pred_file
        return input_dict


@DATASETS.register_module()
class MultiSweepWaymoDatasetWithPredOCC(WaymoDatasetWithPredOCC):
    def __init__(
            self,
            data_root,
            ann_file,
        ...
        super().__init__(
            ...
            ann_file=ann_file,
            split=split,
            idx2contextname_file=idx2contextname_file,
            idx2timestamp_file=idx2timestamp_file,
            occ_pred_path=occ_pred_path,
            pts_prefix=pts_prefix,
            pipeline=pipeline,
            classes=classes,
            modality=modality,
