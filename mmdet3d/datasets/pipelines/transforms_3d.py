...
from .data_augment_utils import box3d_transform_, noise_per_object_v3_
import torch
    ...
    def __call__(self, input_dict):
        ...
        if sampled_dict is not None:
            ...
            if gt_dim == 9 and sampled_gt_bboxes_3d.shape[1] == 7:
                ...
                gt_np = np.pad(
                    gt_np, ((0, 0), (0, 1)), mode="constant", constant_values=1
                )
                gt_bboxes_3d.box_dim = 10  # update box_dim

            gt_bboxes_3d = gt_bboxes_3d.new_box(
                np.concatenate([gt_np, sampled_gt_bboxes_3d])
            )

            points, mask = self.remove_points_in_boxes(
                points, sampled_gt_bboxes_3d[:, :7], return_mask=True
            )
            # check the points dimension
            if sampled_points.points_dim != points.points_dim:
                # warnings.warn(
                #     "Sampled points and input points have different dimensions"
                # )
                extra_channels = torch.zeros(
                    (sampled_points.shape[0], points.points_dim - sampled_points.points_dim ),
                    dtype=sampled_points.tensor.dtype,
                    device=sampled_points.tensor.device,
                )
                sampled_points.tensor = torch.cat([sampled_points.tensor, extra_channels], dim=1)
                sampled_points.points_dim = points.points_dim
            ...
            if "pts_frame_inds" in input_dict:
                ...
                frame_inds = np.concatenate(
                    [np.zeros(len(sampled_points), dtype=int), frame_inds], 0
                )
...
class BoxNoise(object):
    """Apply noise to each GT bbox in the scene.
    After adding noise, the GT bbox is no longer aligned with the object points.
    Here we use it for occ reconstruction for any ROI
    ...
    def __call__(self, input_dict):
        ...
        # clone
        numpy_box = gt_bboxes_3d.tensor.clone().numpy()
        num_boxes = numpy_box.shape[0]
        rotation_perturb = self.rot_range
        center_noise_std = self.translation_std
        ...
        loc_noises = np.random.normal(
            scale=center_noise_std, size=[num_boxes, self.num_aug, 3]
        )
        rot_noises = np.random.uniform(
            rotation_perturb[0], rotation_perturb[1], size=[num_boxes, self.num_aug]
        )
        # pdb.set_trace()
        numpy_box = np.tile(numpy_box.reshape(num_boxes, 1, -1), (1, self.num_aug, 1))
        if "gt_labels_3d" in input_dict:
            gt_labels_3d_aug = np.tile(
                input_dict["gt_labels_3d"].reshape(num_boxes, 1), (1, self.num_aug)
            )
            gt_labels_3d_aug = gt_labels_3d_aug.reshape(-1)
        numpy_box[..., :3] += loc_noises
        numpy_box[..., 6] += rot_noises
        numpy_box = numpy_box.reshape(num_boxes * self.num_aug, -1)
        if self.keep_origin:
            gt_bboxes_3d = gt_bboxes_3d.cat(
                [gt_bboxes_3d, gt_bboxes_3d.new_box(numpy_box)]
            )
            input_dict["gt_bboxes_3d"] = gt_bboxes_3d
            if "gt_labels_3d" in input_dict:
                input_dict["gt_labels_3d"] = np.concatenate(
                    [input_dict["gt_labels_3d"], gt_labels_3d_aug]
                )
        else:
            input_dict["gt_bboxes_3d"] = gt_bboxes_3d.new_box(numpy_box)
            if "gt_labels_3d" in input_dict:
