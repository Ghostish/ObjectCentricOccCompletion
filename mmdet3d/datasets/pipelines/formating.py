...
@PIPELINES.register_module()
class TrackletOccFormatBundle(TrackletFormatBundle):
     def __call__(self, results):
        ...
        if 'sample_occ_centers' in results and 'sample_occs' in results:
            ...
            if isinstance(sample_occ_centers, list):
                occ_labels = [ torch.cat([c, o.unsqueeze(-1)],dim=-1) for c,o in zip(sample_occ_centers, sample_occs)]
            else:
                occ_labels = torch.cat([sample_occ_centers, sample_occs.unsqueeze(-1)],dim=-1) # N,K,4
            results['occ_labels'] = DC(occ_labels)
        elif 'occ_label' in results:
            results['occ_label'] = DC(results['occ_label'])
