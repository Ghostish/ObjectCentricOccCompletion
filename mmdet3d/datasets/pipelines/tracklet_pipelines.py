...
class LoadTrackletPoints(object):
    def __init__(
        self, load_dim=5, use_dim=5, coord_type="LIDAR", max_points=-1, debug=False
    ):
    ...
    def __call__(self, input_dict):
        ...
        if self.debug:
            ...
            points_list = [
                np.random.rand(100, 6).astype(np.float32) * 2 for i in range(len(trk))
            ]
        ...
        if (
            "point_cloud_interval" in input_dict
            and input_dict["point_cloud_interval"] is not None
        ):
        ...
        frame_inds_list = [
            torch.ones(len(p), dtype=torch.int) * i for i, p in enumerate(points_list)
        ]  # continuous frame_inds
        # points = np.concatenate(points_list, 0)
        # points_class = get_points_type(self.coord_type)
        # points = points_class(points, points_dim=points.shape[-1], attribute_dims=None)
        if self.max_points > 0:
            points_list, frame_inds_list = self.points_downsample(
                points_list, frame_inds_list
            )
...
class LoadOccAnnotations(object):
    ...
    def __call__(self, input_dict):
        anno_dict = input_dict["ann_info"]
        occ_label = np.load(anno_dict["occ_label_name"])["occ"]
        input_dict["occ_label"] = torch.from_numpy(occ_label)
...
class TrackletCutting(object):
    def __init__(
        ...
        max_cut_ratio=0.5,
        max_length=200,
        shift_time_zero=False,
    ):
        self.min_length = min_length
        self.ratio = ratio
        self.max_cut_ratio = max_cut_ratio
        self.max_length = max_length
        self.shift_time_zero = shift_time_zero

    def __call__(self, input_dict):
        tracklet = input_dict["tracklet"]
        if len(tracklet) < self.min_length or (
            np.random.rand() > self.ratio and len(tracklet) < self.max_length
        ):
            return input_dict
        ...
        if self.shift_time_zero:
            pts_frame_inds = [
                torch.ones(len(p), dtype=torch.int) * i for i, p in enumerate(points_list)
            ]  # shift frame_inds such that the first frame is 0
        else:
...
@PIPELINES.register_module()
class TrackletRegularization(object):
    """
    Pad or Cut the tracklet to a fixed length
    """

    def __init__(self, reg_len=150):
        self.reg_len = reg_len

    def __call__(self, input_dict) -> Any:
        tracklet = input_dict["tracklet"]
        if len(tracklet) == self.reg_len:
            return input_dict
        points_list = input_dict["points"]
        pts_frame_inds = input_dict["pts_frame_inds"]
        ts_list = tracklet.ts_list
        if len(tracklet) < self.reg_len:
            ...
            # print(f"tracklet length {len(tracklet)} < {self.reg_len}")
            warnings.warn(f"tracklet length {len(tracklet)} < {self.reg_len}")
            pad_len = self.reg_len - len(tracklet)
            pad_ts = [ts_list[-1]] * pad_len
            pad_box = [tracklet.box_list[-1]] * pad_len
            pad_score = [tracklet.score_list[-1]] * pad_len
            pad_pose = [tracklet.pose_list[-1]] * pad_len
            tracklet.box_list.extend(pad_box)
            tracklet.ts_list.extend(pad_ts)
            tracklet.score_list.extend(pad_score)
            tracklet.pose_list.extend(pad_pose)
            tracklet.size += pad_len
            points_list = points_list + [points_list[-1]] * pad_len
            pts_frame_inds = pts_frame_inds + [pts_frame_inds[-1]] * pad_len

        else:
            cut_len = len(tracklet) - self.reg_len
            head = np.random.randint(0, cut_len)
            tail = cut_len - head
            cut_ts = ts_list[:head] + ts_list[-tail:]
            ...
            frame_inds_list = [
                torch.ones(len(p), dtype=torch.int) * i for i, p in enumerate(points_list)
            ]  # reconstruct continuous frame_inds
            pts_frame_inds = frame_inds_list
            # pts_frame_inds = pts_frame_inds[head:-tail]
            tracklet.remove(cut_ts)
...
class TrackletPoseTransform(object):
    ...
    def __call__(self, input_dict):
        ...
        tracklet.frame_transform(
            center_pose
        )  # transform each frame to world coord, then center the tracklet to the middle frame's ego pose
        ...
        points_list = [
            torch.cat(
                [
                    self.points_frame_transform(p[:, :3], pose, None, tgt_pose_inv),
                    p[:, 3:],
                ],
                1,
            )
        ...
        if (
            self.centering
        ):  # centering the tracklet to the middle frame's bbox; translation only
...
class TrackletRandomFlip(object):
    ...
    def __init__(
        self, flip_ratio_bev_horizontal=0.0, flip_ratio_bev_vertical=0.0, **kwargs
    ):
...
class PointDecoration(object):
    # decorate each point with extra attributes (e.g., box yaw, size, and score)
    ...
    def yaw(self, points_list, trk):
        assert len(points_list) == len(trk), f"{len(points_list)}, {len(trk)}"
        box_list = trk.box_list
        yaw_list = [b.tensor[0, 6].item() for b in box_list]
        points_list = [
            F.pad(p, (0, 1), "constant", yaw / 3.1415)
            for p, yaw in zip(points_list, yaw_list)
        ]
        return points_list
    ...
    def size(self, points_list, trk):
        ...
        points_list = [
            torch.cat([p, s.expand(len(p), -1)], 1)
            for p, s in zip(points_list, size_list)
        ]
        return points_list
    ...
    def score(self, points_list, trk):
        ...
        points_list = [
            F.pad(p, (0, 1), "constant", s) for p, s in zip(points_list, score_list)
        ]
        return points_list
...
class TrackletScaling(object):
    ...
    def __call__(self, input_dict):
        if (
            np.random.rand() > self.ratio
            or len(input_dict["tracklet"]) < self.median_filter_size
        ):
            return input_dict
